...
import os, os.path, datetime, time, sys, psutil
import ConfigParser
...
class MySyncHandler(FileSystemEventHandler):
    ...
    def on_created(self, event):
        ...
        firmwarePath = os.path.join( filename, "ALC5521_Firmware/" )
        #display("Syncing %s " % filename)
        if os.path.isdir( firmwarePath ):
...
def main():
   ...
   # load_config();
    ...
    observer = Observer()
    observer.schedule(MySyncHandler(), sys.argv[1], recursive=True)
    observer.start()
    try:
        while True:
            time.sleep(1)
